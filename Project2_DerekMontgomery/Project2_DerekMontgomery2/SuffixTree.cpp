...
SuffixTree::SuffixTree(vector<char>& s1, string alphabet)
{
	this->numNodes = 0;
	this->numLeafs = 0;
...
SuffixNode* SuffixTree::findPath(SuffixNode* T, char* suff) 
{
	int i = 0, j = 0;
	...
	for (i = 0; i < T->getChildrenPointers()->size(); i++) 
	{
		...
		if (*(child->getLinkLabel()) == *suff) //First Character Matches
		{
			...
			else if (j == child->getDepth()) //Node link used -- need to recursively iterate further into the tree
			{
				...
				SuffixNode *newLeaf = findPath(child, remainingSuff);
				return newLeaf;
			}
		}
...
SuffixNode* SuffixTree::createTree(SuffixNode* U, char* suff) // insertion but now with suffix links -- assume root already created and first node passed
{
	...
	for (int i = 0; i < strlen(suff); i++)
	{
		...
		else
		{
			if (localU->getParent() != this->root) // Case 2A SL(U) is unknown and U' is not the root
			{
				...
				V = NodeHop(Vprime, beta, betaLength, pathToFind);
				localU->setLink(V);
			...
			else // Case 2B SL(U) is unkown and U' is the root
			{
				...
				V = NodeHop(this->root, betaPrime, localU->getDepth() - 1, &cp[i]);
...
SuffixNode* SuffixTree::NodeHop(SuffixNode* t, char* path, int pathLen, char *s) //Naive Find path alg
{
	...
	while (it != tChildren->end())
	{
		...
		if (*temp == *cp) // first character match
		{
			...
			else if(VDepth == pathLen)
			{
				...
				s = s + VDepth;
				return NodeHop((*it), cp, pathLen, s);
			}
